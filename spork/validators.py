...
def is_spec_kit_initialized(repo_path: Path, main_branch: str) -> ValidationResult:
    ...
    """
    # First check if Spec Kit exists on the main branch
    ...
    try:
        ...
        if result.returncode != 0:
            return ValidationResult(
                ...
                suggestion=f"Ensure Spec Kit is initialized and committed to {main_branch} branch before creating feature worktrees"
            )
        ...
        try:
            ...
            if result.returncode != 0 or not result.stdout.strip():
                ...
                return ValidationResult(
                    check_name="spec_kit_on_main",
                    passed=False,
                    error_message=f"Spec Kit incomplete on {main_branch} branch ({path_name} missing)",
